...
class Trainer(BaseTrainer):
    def __init__(self, agent, envs_train, envs_eval, 
        ...
        self.saved_path = "./results/" + args.morphologies[0] + "/critic_type_" + args.critic_type + '_actor_type_' + args.actor_type + '_seed_' + str(args.seed) + '/'
        ...
        for step in tqdm(range(self.warmup_timesteps)):
            ...
            for i in range(self.args.num_envs_train):
                ...
                if not done_list[i]:
                    ...
                    obs = np.array(obs_list[i]).astype(np.float32)
                    new_obs = np.array(new_obs_list[i]).astype(np.float32)
    ...
    def train(self):
        ...
        save_interval = max(int(self.max_epoch/200), 1)
        
        for epoch in trange(self.max_epoch, colour='blue', desc='outer loop'):
            
            epoch_start_time = time()
            
            stored_epoch, stored_return = self.my_post_step(epoch)
            if stored_epoch!= None:
                stored_epochs.append(stored_epoch)
                stored_returns.append(stored_return)
                        
            if epoch%save_interval ==0:
                cur_results = {'epochs':stored_epochs, 'returns':stored_returns}
                saved_path = self.saved_path + str(epoch)
                if not os.path.exists(saved_path):
                    os.makedirs(saved_path)
                with open(saved_path + '/results.json', 'w') as f:
            ...
            for env_step in trange(self.num_env_steps_per_epoch, colour='green', desc='inner loop'):
                ...
                '''
                stored_step, stored_return = self.my_post_step(self.tot_env_steps)  
                if stored_step!= None:
        ...
        results = {'epochs':stored_epochs, 'returns':stored_returns}
        saved_path = self.saved_path + 'final/'
        if not os.path.exists(saved_path):
            os.makedirs(saved_path)
        with open(saved_path + 'results.json', 'w') as f:
            json.dump(results, f)
