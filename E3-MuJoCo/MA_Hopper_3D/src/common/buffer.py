...
class ReplayBuffer(object):
    def __init__(self, obs_space, action_space, max_buffer_size = 1000000, modular=False, **kwargs):
        ...
        self.obs_buffer = np.zeros((max_buffer_size, self.obs_dim)).astype(np.float32)
        if self.discrete_action:
            self.action_buffer = np.zeros((max_buffer_size, )).astype(np.long)
        else:
            self.action_buffer = np.zeros((max_buffer_size, self.action_dim)).astype(np.float32)
        self.next_obs_buffer = np.zeros((max_buffer_size,self.obs_dim)).astype(np.float32)
        self.reward_buffer = np.zeros((max_buffer_size,)).astype(np.float32)
        self.done_buffer = np.zeros((max_buffer_size,)).astype(np.float32)
