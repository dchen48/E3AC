...
class DeterministicActor(nn.Module):
    ...
    def __init__(self, feature_dim, action_dim, max_action, hidden_dim = 256):
...
class E3Actor(torch.nn.Module):
    def __init__(self,
        ...
        self.net = SEGNN(
            ...
            output_irreps = Irreps("3x0e"), # 3D continous action
    ...
    def normalize_u(self,u):
        ...
        normalized_action = self.max_action * torch.tanh(u)
        #normalized_action[:,0:3] = 0
        #dummy_action = torch.zeros((len(normalized_action),3)).to(normalized_action.device)
        #normalized_action = torch.cat((dummy_action, normalized_action), dim = -1)
...
class ActorGraphPolicy(nn.Module):
    ...
    def zero_func_bu(self):
        return torch.zeros((1, self.batch_size, self.msg_dim), requires_grad=True).to(util.device)
